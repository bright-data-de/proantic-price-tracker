"""
Proantic Price Tracker - Bright Data Web Scraper API
----------------------------------------------------
Track prices, availability, and product data from Proantic.

Docs  : https://docs.brightdata.de/scraping-automation/web-scraper-api/overview
Info  : https://brightdata.de/products/insights/price-tracker/proantic
"""
...
# Set your Web Scraper ID after building your Proantic scraper:
# https://brightdata.de/products/web-scraper/proantic
DATASET_ID = os.getenv("BRIGHTDATA_DATASET_ID", "")
if not DATASET_ID:
    raise EnvironmentError(
        "BRIGHTDATA_DATASET_ID is not set.\n"
        "1. Build your Proantic scraper at: https://brightdata.de/products/web-scraper/proantic\n"
...
def track_prices(urls: list[str], **kwargs) -> list[dict]:
    """
    Collect price data for a list of Proantic product URLs.
...
def discover_by_keyword(keyword: str, limit: int = 50, **kwargs) -> list[dict]:
    """
    Discover Proantic products by keyword search.
...
def discover_by_category(category_url: str, limit: int = 100, **kwargs) -> list[dict]:
    """
    Discover all products from a Proantic category page.

    Example:
        results = discover_by_category("https://proantic.com/s?k=headphones", limit=200)
    """
...
if __name__ == "__main__":
    ...
    sample_urls = [
        "https://www.proantic.com/product/sample-item-123456",
        # Add more Proantic product URLs here
    ]
    results = track_prices(sample_urls)

    output_file = "proantic_prices.json"
    ...
    # cat_results = discover_by_category("https://proantic.com/category/sample", limit=50)
